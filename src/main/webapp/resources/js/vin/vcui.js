...
;(function (core, global, undefined) {
    ...
    core.addon('string', function () {
        var escapeChars = {
            ...
            hexRegexp = /^\&#x([\da-fA-F]+);$/,
            utf8ByteLength = document.charset.toLowerCase() === 'utf-8' ? 3 : 2;
        ...
        return /** @lends vcui.string */{
            ...
            byteLength: function (value) {
                ...
                for (var i=0; i<value.length; ++i) {
                    (value.charCodeAt(i) > 127) ? byte += utf8ByteLength : byte++;
                }
            ...
            indexByByte: function (value, length) {
                ...
                for (len = i = 0; c = value.charCodeAt(i++);) {
                    len += c >> 11 ? utf8ByteLength : c >> 7 ? 2 : 1;
                    if (len > length) {
                        return i > 0 ? i - 1 : 0;
                    }
                }
...
(function ($, core, context, undefined) {
     ...
     * @return {jQuery}
     */
    $.fn.activeItem = function (className, flag) {
        className = className || 'on';
        if (typeof  flag === 'undefined') {
            flag = true;
        }
        this.toggleClass(className, flag);

        if (flag) {
            this.siblings().toggleClass(className, !flag).end();
        }
        return this;
    };
